...
from .tuners import LoraConfig, PrefixTuningConfig, PromptEncoderConfig, PromptTuningConfig, BottleneckConfig, DoraConfig, SARAConfig
...
TRANSFORMERS_MODELS_TO_SARA_TARGET_MODULES_MAPPING = {
    "t5": ["q", "v"],
    "mt5": ["q", "v"],
    "bart": ["q_proj", "v_proj"],
    "gpt2": ["c_attn"],
...
def _prepare_sara_config(peft_config, model_config):
    """
    Prepares the SARA configuration by setting target modules if not already set.
    """
    if peft_config.target_modules is None:
        model_type = model_config.get("model_type", "").lower()
        if model_type not in TRANSFORMERS_MODELS_TO_SARA_TARGET_MODULES_MAPPING:
            raise ValueError("Please specify `target_modules` in `peft_config` for SARA.")
        peft_config.target_modules = TRANSFORMERS_MODELS_TO_SARA_TARGET_MODULES_MAPPING[model_type]
    if peft_config.inference_mode:
        peft_config.merge_weights = True
    return peft_config    
...
def get_peft_model(model, peft_config):
    ...
    if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
        if peft_config.peft_type == "LORA":
            ...
            return PeftModel(model, peft_config)
        elif peft_config.peft_type == "SARA":  # Handle SARA
            peft_config = _prepare_sara_config(peft_config, model_config)
            return PeftModel(model, peft_config)
    else:
        ...
        # Map task type and peft type to the corresponding PeftModel
        return MODEL_TYPE_TO_PEFT_MODEL_MAPPING.get(
            f"{peft_config.peft_type}_{peft_config.task_type}",
            PeftModel(model, peft_config),
        )
