...
class SARAModel(BaseTuner):
    ...
    Args:
        ...
        config ([`SARAConfig`]): The configuration of the SARA model.
        adapter_name (`str`): The name of the adapter, defaults to `"default"`.
        low_cpu_mem_usage (`bool`, `optional`, defaults to `False`):
            Create empty adapter weights on meta device. Useful to speed up the loading process.

    Returns:
        `torch.nn.Module`: The SARA model.

    Example:

        ```py
        >>> from transformers import AutoModelForCausalLM
        >>> from peft import SARAConfig, get_peft_model

        >>> base_model = AutoModelForCausalLM.from_pretrained("facebook/opt-125m")
        >>> config = SARAConfig(r=8)
        >>> model = get_peft_model(base_model, config)
        ```

    **Attributes**:
        - **model** ([`~transformers.PreTrainedModel`]) -- The model to be adapted.
        - **peft_config** ([`SARAConfig`]): The configuration of the SARA model.
    """
    ...
    def _find_dim(self, config) -> tuple[int, int]:
        """
        Finds the largest input and output dimensions across linear layers that have been wrapped with SARA.

        This will be used for determining the size of the shared sara_A and sara_B matrices.
        """
        ...
        if largest_shape is None:
            msg = "No layers types compatible with SARA were found. Please check `peft_config.target_modules`."
            raise ValueError(msg)

        return largest_shape

    def _init_sara_A_sara_B(self, config: SARAConfig, adapter_name: str) -> None:
        ...
        self.sara_A = BufferDict({}, persistent=config.save_projection)
        self.sara_B = BufferDict({}, persistent=config.save_projection)

        # deterministic init of vera_A and vera_B if we know the key
        generator = torch.Generator(device="cpu").manual_seed(config.projection_prng_key)
        sara_A = _kaiming_init((config.r, linear_in_dim), generator=generator)
        sara_B = _kaiming_init((linear_out_dim, config.r), generator=generator)

        self.sara_A[adapter_name] = sara_A
        self.sara_B[adapter_name] = sara_B

    def _pre_injection_hook(self, model: nn.Module, config: SARAConfig, adapter_name: str) -> None:
        self._init_sara_A_sara_B(config, adapter_name)

    def _check_new_adapter_config(self, config: SARAConfig) -> None:
        """
        ...
        if len(save_project_unique_values) > 1:
            raise ValueError(
                "SARA projection weights must be saved for all adapters or none, but got multiple different values: "
                f"{save_project_unique_values}"
            )

    @staticmethod
    def _check_target_module_exists(sara_config, key):
        return check_target_module_exists(sara_config, key)
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        kwargs = {
            "r": r,
            "sara_dropout": sara_config.sara_dropout,
            "sara_alpha": sara_config.sara_alpha,
            "sara_c": sara_config.sara_c,
            "fan_in_fan_out": sara_config.fan_in_fan_out,
            "init_weights": sara_config.init_sara_weights,
        ...
        if isinstance(target, Linear):
            target.update_layer(
                ...
                self.sara_B,
                r,
                sara_config.sara_dropout,
                sara_config.init_sara_weights,
                sara_c = sara_config.sara_c,
                sara_alpha = sara_config.sara_alpha,
                sara_lambda_initial = sara_config.sara_lambda_initial
            )

        else:
            new_module = self._create_new_module(sara_config, self.sara_A, self.sara_B, adapter_name, target, **kwargs)
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, SARALayer) and hasattr(m, "bias") and m.bias is not None:
                        m.bias.requires_grad = True
            else:
                raise NotImplementedError(f"Requested bias: {bias}, is not implemented.")

    @staticmethod
    def _create_new_module(sara_config, sara_A, sara_B, adapter_name, target, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target_base_layer, bnb.nn.Linear8bitLt):
            ...
            return Linear8bitLt(target, adapter_name, sara_A, sara_B, **eightbit_kwargs)
            ...
            return Linear4bit(target, adapter_name, sara_A, sara_B, **fourbit_kwargs)
            ...
            if kwargs["fan_in_fan_out"]:
                ...
                kwargs["fan_in_fan_out"] = sara_config.fan_in_fan_out = False
            ...
            if not kwargs["fan_in_fan_out"]:
                ...
                kwargs["fan_in_fan_out"] = sara_config.fan_in_fan_out = True
        else:
            raise ValueError(
        ...
        new_module = Linear(
            ...
            adapter_name,
            bias=bias,
            sara_lamnda_initial=sara_config.sara_lambda_initial,
            **kwargs,
        )
    ...
    def set_adapter(self, adapter_name):
        for module in self.model.modules():
            if isinstance(module, SARALayer):
                if module.merged:
    ...
    def _unload_and_optionally_merge(
        ...
        key_list = [key for key, _ in self.model.named_modules() if "sara" not in key]
    ...
    def delete_adapter(self, adapter_name: str):
        ...
        for key in key_list:
            ...
            if isinstance(target, SARALayer):
